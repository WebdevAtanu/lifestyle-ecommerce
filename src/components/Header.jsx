...
function Header() {
	...
	return (
		...
		</div>
		{
			...
			<input type="text" value={data} onChange={(e)=>handleChange(e)} className='py-2 px-5 outline-none w-full md:w-1/2 border border-gray-500 border-r-0' placeholder='Search Product'/>
			<button onClick={()=>setData('')} className='bg-white border border-gray-500 border-l-0 px-3 text-gray-400'><i className="bi bi-x-lg"></i></button>
			</div>:null
		}
